...
====================================
This guide will show you how to build koipayd (headless client) for OSX.
...
5.  (Optional) You can also install koipayd to your path:
...
4. Enter "koipay-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `koipayd` for your own use.

koipayd/koipay-cli binaries are not included in the koipay-Qt.app bundle.

If you are building `koipayd` or `koipay-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the KoiPay-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./koipayd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./koipayd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=koipayrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/KoiPay/koipay.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/KoiPay/koipay.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/KoiPay/debug.log

Other commands:
-------

    ./koipayd -daemon # to start the koipay daemon.
    ./koipay-cli --help  # for a list of command-line options.
    ./koipay-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure KoiPay with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the KoiPay wallet:
...
You have the choice to build the GUI KoiPay wallet as a Mac OSX app, described in “How to build the KoiPay-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the KoiPay-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find KoiPay-Qt.dmg inside your KoiPay folder. Open and install the wallet like any typical Mac app.
...
~/Downloads/KoiPay/src/qt/koipay-qt