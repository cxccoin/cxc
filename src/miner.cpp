...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zCXC spends can have very large priority, use non-overflowing safe functions
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("KoiPayMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("KoiPayMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("KoiPayMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("koipay-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running KoiPayMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
