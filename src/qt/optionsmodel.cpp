...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CXC);
    ...
    if (!settings.contains("nAnonymizeKoiPayAmount"))
        settings.setValue("nAnonymizeKoiPayAmount", 1000);

    nAnonymizeKoiPayAmount = settings.value("nAnonymizeKoiPayAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeKoiPayAmount"))
        SoftSetArg("-anonymizekoipayamount", settings.value("nAnonymizeKoiPayAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in koipay.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKoiPayAmount:
            return QVariant(nAnonymizeKoiPayAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKoiPayAmount:
            nAnonymizeKoiPayAmount = value.toInt();
            settings.setValue("nAnonymizeKoiPayAmount", nAnonymizeKoiPayAmount);
            emit anonymizeKoiPayAmountChanged(nAnonymizeKoiPayAmount);
            break;
        case CoinControlFeatures:
