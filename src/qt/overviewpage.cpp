...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CXC)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, QString& sCXCPercentage)
{
    int nPrecision = 2;
    ...
    sCXCPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelCXCPercent->setText(sPercentage);
    ...
    bool showCXCAvailable = settingShowAllBalances || phrAvailableBalance != nTotalBalance;
    bool showWatchOnlyCXCAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showCXCPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyCXCPending = watchUnconfBalance != 0;
    bool showCXCLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyCXCLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showCXCAvailable || showWatchOnlyCXCAvailable);
    ui->labelBalanceText->setVisible(showCXCAvailable || showWatchOnlyCXCAvailable);
    ui->labelWatchAvailable->setVisible(showCXCAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showCXCPending || showWatchOnlyCXCPending);
    ui->labelPendingText->setVisible(showCXCPending || showWatchOnlyCXCPending);
    ui->labelWatchPending->setVisible(showCXCPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showCXCLocked || showWatchOnlyCXCLocked);
    ui->labelLockedBalanceText->setVisible(showCXCLocked || showWatchOnlyCXCLocked);
    ui->labelWatchLocked->setVisible(showCXCLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("CXC")
    updateDisplayUnit();
}
