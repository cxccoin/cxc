...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"koipayprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"koipayprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsHex(params[0].get_str())) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid KoiPay address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"koipayaddress\"\n"
            "\nReveals the private key corresponding to 'koipayaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"koipayaddress\"   (string, required) The koipay address for the private key\n"
            "\nResult:\n"
...
UniValue dumpallprivatekeys(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nSCAM WARNING: If anyone asks you to run this command and send them the file,\n"
            "they will have FULL ACCESS to STEAL your CXC. Giving this file to someone\n"
            "is the same thing as giving them all of the CXC in your wallet! Never send\n"
            "this file to ANYONE that you do not trust with all of your CXC!!!\n"
           "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename\n"
            "\nExamples:\n" +
    ...
    std::sort(vKeyBirth.begin(), vKeyBirth.end());
    // produce output
    file << strprintf("# Wallet dump created by KoiPay %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    file << strprintf("# * Created on %s\n", EncodeDumpTime(GetTime()));
    file << strprintf("# * Best block at time of backup was %i (%s),\n", chainActive.Height(), chainActive.Tip()->GetBlockHash().ToString());
    file << strprintf("#   mined on %s\n", EncodeDumpTime(chainActive.Tip()->GetBlockTime()));
    file << "\n";
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"koipayaddress\"\n"
            "\nEncrypts a private key corresponding to 'koipayaddress'.\n"
            "\nArguments:\n"
            "1. \"koipayaddress\"   (string, required) The koipay address for the private key (you must hold the key already)\n"
