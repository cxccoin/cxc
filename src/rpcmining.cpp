...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or koipay.conf setting gen)\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty() && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "KoiPay is not connected!");

    if (IsInitialBlockDownload() && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "KoiPay is downloading blocks...");
