...
#if defined(NDEBUG)
#error "KoiPay cannot be compiled without assertions."
#endif
...
{
    int64_t nSubsidy = 0;
    if (nHeight == 0) { nSubsidy = 9300000 * COIN;} 
    else if (nHeight > 0 && nHeight <= 10080) { nSubsidy = 1 * COIN; }
    else if (nHeight <= 4204800 && nHeight > 10080) { nSubsidy = 5 * COIN; }
    else if (nHeight <= 4921121 && nHeight > 4204800) { nSubsidy = 1 * COIN; }
    else if (nHeight > 4921121) { nSubsidy = 0; }
    return nSubsidy;
}
...
{
    RenameThread("koipay-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateCXCSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/KoiPay Core:1.1.0/" ||
            pfrom->cleanSubVer == "/KoiPay Core:1.3.0/" ||
            pfrom->cleanSubVer == "/KoiPay Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // KoiPay: We use certain sporks during IBD, so check to see if they are
