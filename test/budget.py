...
class Node :
	def __init__(self, app, nodeIndex) :
		...
		self._koipayd = self._app.getMonetaryunitd()
		self._koipayCli = self._app.getMonetaryunitCli()
		self._daemonProcess = None

	def createDataDir(self, nodeCount, masterNodePrivateKey = None) :
		makeDirs(self._nodePath)
		writeFile(
			os.path.join(self._nodePath, 'koipay.conf'),
			self._generateMonetaryunitConf(nodeCount, masterNodePrivateKey)
		)
		
	def startNode(self) :
		self._daemonProcess = subprocess.Popen([ self._koipayd, '-datadir=' + self._nodePath, '-daemon' ])
	...
	def executeCli(self, *args) :
		...
		output = executeCommand(self._koipayCli, '-datadir=' + self._nodePath, *normalizedArgs)
...
class Application :
	...
	def _setup(self) :
		...
		self._koipayd = os.getenv('KOIPAYD', None)
		if not self._koipayd :
			die('Undefined KOIPAYD')
		self._koipayCli = os.getenv('KOIPAYCLI', None)
		if not self._koipayCli :
			die('Undefined KOIPAY')
		print('koipayd: %s' % (self._koipayd))
	...
	def getMonetaryunitd(self) :
		return self._koipayd

	def getMonetaryunitCli(self) :
		return self._koipayCli
