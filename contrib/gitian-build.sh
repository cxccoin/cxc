...
VERSION=
commit=false
url=https://github.com/koipayproject/koipay
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the koipay, gitian-builder, gitian.sigs, and koipay-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/koipayproject/koipay
...
then
	# Make output folder
	mkdir -p "./koipay-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit koipay=${COMMIT} --url koipay=${url} ../koipay/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../koipay/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/koipay-*.tar.gz build/out/src/koipay-*.tar.gz ../koipay-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit koipay=${COMMIT} --url koipay=${url} ../koipay/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/koipay-*.zip build/out/koipay-*.exe ../koipay-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit koipay=${COMMIT} --url koipay=${url} ../koipay/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/koipay-*.tar.gz build/out/koipay-*.dmg ../koipay-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit koipay=${COMMIT} --url koipay=${url} ../koipay/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../koipay/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/koipay-*.tar.gz build/out/src/koipay-*.tar.gz ../koipay-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../koipay/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/koipay-*win64-setup.exe ../koipay-binaries/${VERSION}
	    mv build/out/koipay-*win32-setup.exe ../koipay-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../koipay/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/koipay-osx-signed.dmg ../koipay-binaries/${VERSION}/koipay-${VERSION}-osx.dmg
	fi
	popd || exit
